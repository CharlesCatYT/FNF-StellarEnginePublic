...
class CharacterEditorState extends MusicBeatState
{
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var repositioncrap = -200 + OFFSET_X - playerXDifference;

			var bgSchool:BGSprite = new BGSprite('weeb/weebSchool', repositioncrap, -playerYDifference + 6, 0.6, 0.90);
			bgLayer.add(bgSchool);
			bgSchool.antialiasing = false;

			var bgStreet:BGSprite = new BGSprite('weeb/weebStreet', repositioncrap, -playerYDifference, 0.95, 0.95);
			bgLayer.add(bgStreet);
			bgStreet.antialiasing = false;

			var widcrap = Std.int(bgSky.width * 6);
			var bgTrees:FlxSprite = new FlxSprite(repositioncrap - 380, -800 - playerYDifference);
	...
	override function update(elapsed:Float)
	{
		...
		if(!charDropDown.dropPanel.visible) {
			...
			if (FlxG.keys.pressed.SHIFT) camFollow.screenCenter();
		...
		if (FlxG.mouse.wheel != 0) FlxG.camera.zoom += FlxG.mouse.wheel / 10;
	}
