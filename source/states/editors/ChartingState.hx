...
class ChartingState extends MusicBeatState
{
	...
	**/
		...
		var difficultyDropDown = new FlxUIDropDownMenuCustom(stageDropDown.x, gfVersionDropDown.y, FlxUIDropDownMenuCustom.makeStrIdLabelArray(CoolUtil.difficulties, true), function(difficulty:String)
		{
			if (PlayState.storyDifficulty != Std.parseInt(difficulty)) {
				...
				try {
					...
					trace ("File " + _song.song.toLowerCase() + Difficulty.getFilePath() + " is not found.");
				}
			}
		});
	...
	{
		while (thefunnerUI.members.length > 0)
		{
			thefunnerUI.remove(thefunnerUI.members[0], true);
		}

		var title:FlxText = new FlxText(UI_box.x + 20, UI_box.y + 20, 0);
		thefunnerUI.add(title);
	}
	...
	{
		// crapty null fix, i fricking hate it when this happens
	...
	private function saveLevel()
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.save(data.trim(), Paths.formatToSongPath(_song.song) + (Difficulty.getFilePath() == null ? Difficulty.getFilePath() : '') + ".json");
		}
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.justPressed)
		{
			...
			else
			{
				if (FlxG.mouse.x > gridBG.x
					&& FlxG.mouse.x < gridBG.x + gridBG.width
					&& FlxG.mouse.y > gridBG.y
					&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * getSectionBeats() * 4) * zoomList[curZoom]) addNote();
			}
		}
		...
		if (!blockInput)
		{
			...
			if (FlxG.mouse.wheel != 0)
			{
				...
				if (!mouseQuant) FlxG.sound.music.time -= (FlxG.mouse.wheel * Conductor.stepCrochet*0.8);
				else
					{
						...
						if (FlxG.mouse.wheel > 0)
						{
							var frick:Float = CoolUtil.quantize(beat, snap) - increase;
							FlxG.sound.music.time = Conductor.beatToSeconds(frick);
						}else{
							var frick:Float = CoolUtil.quantize(beat, snap) + increase;
							FlxG.sound.music.time = Conductor.beatToSeconds(frick);
						}
			...
			// Vortex stuff, why call it beta, it works fine
			...
			if(!vortex){
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN  )
				{
					...
					{
						var frick:Float = CoolUtil.quantize(beat, snap) - increase; //(Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(frick);
					}else{
						var frick:Float = CoolUtil.quantize(beat, snap) + increase; //(Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(frick);
			...
			if(vortex && !blockInput){
				...
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN  )
				{
					...
					{
						var frick:Float = CoolUtil.quantize(beat, snap) - increase;
						feces = Conductor.beatToSeconds(frick);
					}else{
						var frick:Float = CoolUtil.quantize(beat, snap) + increase; //(Math.floor((beat+snap) / snap) * snap);
						feces = Conductor.beatToSeconds(frick);
					}
	...
	function reloadGridLayer() {
		...
		#if desktop
		if(FlxG.save.data.chart_waveformInst || FlxG.save.data.chart_waveformVoices) updateWaveform();
		#end
