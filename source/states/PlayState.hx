...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(boyfriendCameraOffset == null) //Fricks sake should have done it since the start :rolling_eyes:
	...
	{
		if(ClientPrefs.data.ghostTapping) return; //frick it
	...
	override function beatHit()
	{
		...
		if (ClientPrefs.data.iconBop == "OS") {
			if (dancingLeft){
				iconP1.angle = 8;
				iconP2.angle = 8;
			} else {
	...
	function fullComboUpdate()
	{
		...
		var awfuls:Int = ratingsData[3].hits;

		ratingFC = 'Clear';
		if(songMisses < 1)
		{
			if (bads > 0 || awfuls > 0) ratingFC = 'FC';
