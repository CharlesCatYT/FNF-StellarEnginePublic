...
class Tank extends BaseStage
{
		...
		if(!ClientPrefs.data.lowQuality)
		{
			for (daGf in gfGroup)
			{
				...
				if(gf.curCharacter == 'pico-speaker')
				{
					...
					firstTank.resetcrap(20, 600, true);
					...
					for (i in 0...TankmenBG.animationNotes.length)
					{
						if(FlxG.random.bool(16)) {
							...
							tankBih.resetcrap(500, 200 + FlxG.random.int(50, 100), TankmenBG.animationNotes[i][1] < 2);
	...
	function stressIntro()
	{
		...
		// theres swear words here so beware that, but dont worry we aint using the base game stages anyway
		...
		cutsceneHandler.timer(15.2, function()
		{
			...
			gfCutscene.alpha = 1;
			gfCutscene.animation.play('dieBitch', true); // the b word will be left over here since i wont be using this stage lol
			gfCutscene.animation.finishCallback = function(name:String)
			{
				if(name == 'dieBitch') //Next part
				{
					gfCutscene.animation.play('getRektLmao', true); // this would break the tank stage so i cant change this waaaa
