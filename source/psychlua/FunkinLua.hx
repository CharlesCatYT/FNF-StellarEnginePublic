...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		//little stuff for epic coders like me B)  *image of obama giving himself a medal*
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(game.modchartSprites.exists(tag)) {
				var crap:ModchartSprite = game.modchartSprites.get(tag);
				if(front)
					LuaUtils.getTargetInstance().add(crap);
				else
				{
					if(!game.isDead)
						game.insert(game.members.indexOf(LuaUtils.getLowestCharacterGroup()), crap);
					else
						GameOverSubstate.instance.insert(GameOverSubstate.instance.members.indexOf(GameOverSubstate.instance.boyfriend), crap);
				}
			}
		});
