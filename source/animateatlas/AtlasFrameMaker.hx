	...
	@:noCompletion static function getFramesArray(t:SpriteMovieClip,animation:String):Array<FlxFrame>
	{
		...
		for (i in t.getFrame(animation)...t.numFrames)
		{
			...
			if (t.currentLabel == animation)
			{
				...
				var bitmapcrap:BitmapData = new BitmapData(Std.int(sizeInfo.width + sizeInfo.x), Std.int(sizeInfo.height + sizeInfo.y), true, 0);
				if (ClientPrefs.data.cacheOnGPU)
				{
					var texture:openfl.display3D.textures.RectangleTexture = FlxG.stage.context3D.createRectangleTexture(bitmapcrap.width, bitmapcrap.height, BGRA, true);
					texture.uploadFromBitmapData(bitmapcrap);
					bitmapcrap.image.data = null;
					bitmapcrap.dispose();
					bitmapcrap.disposeImage();
					bitmapcrap = BitmapData.fromTexture(texture);
				}
				bitmapcrap.draw(t, null, null, null, null, true);
				bitMapArray.push(bitmapcrap);

				if (firstPass)
				{
					frameSize.set(bitmapcrap.width,bitmapcrap.height);
					firstPass = false;
				}
			}
			else break;
